...
export class GerberClass {
    constructor(input) {
        this.toolkit = new tc.ToolkitClass();
        this.size = { X1: 0, Y1: 0, X2: 0, Y2: 0 };
        this.rs274x = [];
        this.count = 0;
        this.result = {};
        this.macroSwitcher = {
            "AD": (param) => { this.parse_rs274x_AD(param); },
            "AM": (param) => { this.parse_rs274x_AM(param); },
            "FS": (param) => { this.parse_rs274x_FS(param); },
            "LP": (param) => { this.parse_rs274x_LP(param); },
            "MO": (param) => { this.parse_rs274x_MO(param); },
            "-": (param) => { this.parse_rs274x_default(param); },
        };
        this.input = input;
    ...
    sorted_by_key() {
        let arr = this.result;
        const ordered = Object.keys(arr).sort().reduce((obj, key) => {
            obj[key] = arr[key];
            return obj;
        }, {});
        return ordered;
    ...
    parse_rs274x(str) {
        ...
        if (func == undefined)
            func = this.macroSwitcher["-"];
        func(fullCmd.slice(3));
        this.rs274x = [];
    }
    parse_rs274x_AD(param) {
        ...
        this.toolkit.addAperture(parseInt(D_code[1]), param.slice(D_code[0].length, -1), this.rs274x);
    }
    parse_rs274x_AM(param) {
        //debug(`parse_rs274x_AM: ${this.rs274x}`);
        this.toolkit.addMacro(param.slice(0, -1), this.rs274x);
    }
    parse_rs274x_FS(param) {
